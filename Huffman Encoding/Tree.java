...
// -------------------------------------------------------------
   ...
   public String getCode(char letter){  //we want the code for this letter
	   ...
	   Order(root, letter," ");    //call an inOrder traversal, starting at the root, looking for this letter
	     return path;     //return the path that results

   }
   private void Order(Node localRoot, char letter, String path)
   { //the path variable tracks the current search path
      ...
      { //if root is null we've gone off the edge of the tree - back up 
          ...
          {
        	  Order(localRoot.leftChild, letter, path+"0"); //go left and add "0" to the current search path
        	  Order(localRoot.rightChild, letter, path+"1");    //go right and add "1" to the current search path
         }
      }
